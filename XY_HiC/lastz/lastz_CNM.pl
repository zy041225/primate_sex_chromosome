...
=head1 Options
 ...
 --chain             parameters for lastz (c=2 or 1): --chain or " ", default " "; 
 --continuity <int>  parameters for lastz, filter < % of alignment columns that are not gaps (0--100), defualt: 0
...
my ($output_dir,$cuts,$lastzStep,$hspthresh,$gappedthresh,$ydrop,$inner,$seed,$scores,$continuity,$format,$chain,$minScore,$linearGap,$tn,$qn,$step,$run,$cpu,$resource,$help);
GetOptions(
	"ouput:s"	=> \$output_dir,
	"cuts:i"	=> \$cuts,
...
my $lastz_para = "--step=$lastzStep --hspthresh=$hspthresh --gappedthresh=$gappedthresh --ydrop=$ydrop --inner=$inner --seed=$seed --format=$format $chain --scores=$Bin/$scores --continuity=$continuity ";
...
if ($step =~ /3/){
	...
	`perl $Bin/qsub-sge.pl --convert no --maxjob $cpu  --resource vf=${resource}G --reqsub $lastz_shell` if ($run eq "qsub");
